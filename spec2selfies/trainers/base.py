...
METRICS_DIR =f"{os.environ['WORK']}/metrics"


DEFAULT_INPUT_SELFIES = "[C][C][Branch1][C][C][O][C][C][C][N][C][C][C][Branch1][#Branch2][C][C][=C][C][=C][C][=N][Ring1][=Branch1][Branch1][=N][N][C][=Branch1][C][=O][C][=C][N][=N][O][Ring1][Branch1][C][C][Ring2][Ring1][Ring2]"
...
class BaseTrainer(transformers.Trainer):
    ...
    def __init__(self, *args, **kwargs):
        wandb.init(project="spec2selfies", mode="offline" if os.getenv("WANDB_MODE") == "offline" else "online")
        super().__init__(*args, **kwargs)
        self.preprocess_logits_for_metrics = preprocess_logits_for_metrics
        self.compute_metrics = self.compute_metrics_func
        self.metric_accuracy = evaluate.load(f"{METRICS_DIR}/accuracy")
        self.metric_bleu = evaluate.load(f"{METRICS_DIR}/sacrebleu")
    ...
    def sanity_decode(self, input_string: str = None, max_length: int = 128):
        ...
        if input_string is None:
            input_string = DEFAULT_INPUT_SELFIES
        self.model.eval()
        print("=" * 16, "Begin trainer sanity check", "=" * 16)

        print (f"Nom du modèle : {self.model.name_or_path}")
        print (f"Nom du tokenizer : {self.tokenizer.name_or_path}")

        if self.embedder is None:
            inputs = self.eval_dataset[0]
        ...
        output_string = self.tokenizer.decode(
            regenerated.flatten(), skip_special_tokens=False
        )
    ...
    def _text_comparison_metrics(
        ...
        set_token_metrics = {
            ...
            "token_set_f1_sem": float(sem(f1s)),
        ...
        gen_metrics = {
            "bleu_score": float(bleu_results.mean()),
            "bleu_score_sem": float(sem(bleu_results)),
            #"rouge_score": rouge_result["rouge1"],
            # ['rouge1', 'rouge2', 'rougeL', 'rougeLsum']
            # "bert_score": statistics.fmean(bertscore_result["f1"]),
            "exact_match": float(mean(exact_matches)),
            "exact_match_sem": float(sem(exact_matches)),
        }
    ...
    def eval_generation_metrics(
        ...
        preds_sample_list, preds_sample_labels_list = self._get_decoded_sequences(
            dataloader=dataloader, n=10_000
        )
        ...
        try:
            with torch.no_grad():
                ...
                sim_result = {
                    ...
                    "emb_cos_sim_sem": float(sem(emb_cos_sims.cpu().numpy())),
                    "emb_top1_equal": emb_topk_equal.mean().item(),
                    "emb_top1_equal_sem": float(sem(emb_topk_equal)),
                }
