...
os.environ["TMPDIR"] = os.getenv("VEC2TEXT_CACHE")
...
def load_experiment_and_trainer(
    ...
    store_embedder: Optional[bool] = None,
    ...
    if store_embedder is not None:
        print(
            f"Overwriting store_embedder from {model_args.store_embedder} to {store_embedder}"
        )
        model_args.store_embedder = store_embedder
    ...
    print ('#' * 16)
    print (f"Printing from load_experiment_and_trainer() utils.py")
    print (experiment)
    print (experiment.model_args)
    print ('#' * 16)
...
def dataset_map_multi_worker(
    ...
    print(f"\tworker {rank} will save sub-shard to {ds_shard_filepaths[rank]}")
    ...
    print("Dataset cache:", config.HF_DATASETS_CACHE)
